...
def perform_request(method, url, data_or_params=None, *args, **kwargs):
    ...
    if 200 < response.status_code < 500:
        raise CommentClientRequestError(response.status_code, response.text)
    # Heroku returns a 503 when an application is in maintenance mode
        ...
        raise CommentClientServerError(response.text)
    else:
...
class CommentClientServerError(CommentClientError):
    pass
