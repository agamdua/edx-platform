...
class User(models.Model):
    ...
    def vote(self, voteable, value):
        if voteable.type == 'thread':
            ...
            raise CommentClientRequestError(400, "Can only vote / unvote for threads or comments")
    ...
    def active_threads(self, query_params={}):
        if not self.course_id:
            raise CommentClientRequestError(400, "Must provide course_id when retrieving active threads for the user")
    ...
    def subscribed_threads(self, query_params={}):
        if not self.course_id:
            raise CommentClientRequestError(400, "Must provide course_id when retrieving subscribed threads for the user")
